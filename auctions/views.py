...
def index(request):
    ...
    latest_auction_list = auction_list.filter(
...
def detail(request, auction_id):
    ...
    auction.resolve()
    already_bid = False
    all_bids = Bid.objects.all().filter(auction=auction).order_by('-date')
    if request.user.is_authenticated:
        if auction.author == request.user:
            own_auction = True
            return render(request, 'auctions/detail.html', {'auction': auction, 'own_auction': own_auction,'all_bids': all_bids})
        ...
        if user_bid:
            ...
            return render(request, 'auctions/detail.html',
                          {'auction': auction, 'already_bid': already_bid, 'bid_amount': bid_amount, 'all_bids': all_bids})

    return render(request, 'auctions/detail.html', {'auction': auction, 'already_bid': already_bid,'all_bids': all_bids})
    # try:
...
def bid(request, auction_id):
    ...
    try:
        ...
        return render(request, 'auctions/my_bids.html',{'my_bids_list':my_bids_list})
...
def my_auctions(request):
    ...
    my_auctions_list = Auction.objects.all().filter(
        author=request.user).order_by('-date_added')
...
def contact(request):
    if request.method == 'POST':
        ...
        contact = Contact(subject=subject, email=email,
...
def withdraw(request):
    if request.method == 'POST':
        ...
        withdraw = Withdrawal(method=method, amount=amount,
                              email=email, user_id=user_id, address=address)
        withdraw.save()
